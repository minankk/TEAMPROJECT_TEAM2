// frontend/src/pages/HipHopPage.js (updated)
...
const HipHopPage = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5001/products/genre/Hip-Hop')
            ...
            .catch(err => console.error('Error loading Hip Hop albums:', err));
    }, []);

    return (
        <main className="hiphop-page">
            <section className="hiphop-products">
                <h2>Hip Hop Albums</h2>
                <div className="hiphop-product-grid">
                    {products.map(product => (
                        <div key={product.product_id} className="hiphop-product-card" onClick={() => openPopup(product.product_id)}>
                            <div className="hiphop-product-container">
                                <img src={`http://localhost:5001${product.cover_image_url}`} alt={product.album_name} className="hiphop-product-image" />
                                <button
                                ...
                                <div className="hiphop-product-info">
                                    <h3>{product.album_name}</h3>
                                    <p>{product.artist_name}</p>
                                    <p>{product.price}</p>
                                </div>
                                <div className="hiphop-product-actions">
