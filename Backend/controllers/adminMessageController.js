...
exports.sendAdminMessage = async (req, res) => {
    try {
        const [adminRows] = await db.query('SELECT role FROM users WHERE user_id = ?', [req.user.user_id]);
        ...
        await db.query(
            'INSERT INTO messages (sender_id, receiver_id, message, sent_at, is_read, parent_id) VALUES (?, ?, ?, NOW(), ?, ?)',
            [req.user.user_id, receiverId, message, false, null] 
        );
...
exports.getAdminMessages = async (req, res) => {
    try {
        ...
        const [messages] = await db.query(
            'SELECT message_id, sender_id, receiver_id, message, sent_at, is_read, parent_id FROM messages WHERE sender_id = ? ORDER BY sent_at DESC',
            [req.user.user_id]
        );
...
exports.deleteAdminMessage = async (req, res) => {
    try {
        ...
        await db.query('DELETE FROM messages WHERE message_id = ? AND sender_id = ?', [messageId, req.user.user_id]);
